...
describe("config.js", () => {
  ...
  it("able to http request",  () => {
      ...
      return  httpRequest.request(headers, requestUrl, body)
      .then((res) => {
  ...
  it("able to http request with string", () => {
    let httpRequest = new HttpRequest(3000);
      ...
      return  httpRequest.request(headers, requestUrl, JSON.stringify(body))
      .then((res) => {
        expect(res.responseCode).toEqual(expect.any(String));
        expect(res.responseCode).toBe("2007300");
        expect(res.responseMessage).toEqual(expect.any(String));
  ...
  it("not able to http request with wrong string format", () => {
    let httpRequest = new HttpRequest(3000);
      ...
      return  httpRequest.request(headers, requestUrl, wrongExample)
      .catch((err) =>{
