...
class Snap {
   ...
   * @param  {String} endPoint - payment method that use, like virtualAccount etc.
   * @param  {String} accessToken - valid access token from requestSnapAccessToken.
   * @param  {String} httpMethod - valid access token from requestSnapAccessToken.
  ...
  requestSnapTransaction(parameter = {}, endPoint, accessToken) {
    ...
    const stringToSign = `POST:${endPoint}:${accessToken}:${hexPayload}:${formattedDate}`;
    ...
    let responsePromise = this.httpClient.request(headers, url, parameter.body);
  ...
  requestSnapAccessToken(parameter = {}) {
    ...
    let responsePromise = this.httpClient.request(headers, url, parameter);
    return responsePromise;
  }
}
